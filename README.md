...
6. If use `./setup.sh -d small` in step 5, the WebShop only loads 1,000 products for a faster environment preview. \
To load all products, use `./setup.sh -d all` in step 5, and change `web_agent_site/utils.py`:
```python
